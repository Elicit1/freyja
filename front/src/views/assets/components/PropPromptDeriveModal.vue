<template>
  <el-dialog
    ...
    class="prop-prompt-derive-dialog"
  >
    <div class="space-y-4 max-h-[72vh] overflow-y-auto pr-1">
      <!-- 模式切换控制器：API 自动生成 vs 复制 Prompt 到外部 AI -->
      <div class="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200/80 rounded-xl p-3 flex items-center justify-between shadow-xs">
        <div class="flex items-center gap-2">
          <span class="text-sm font-bold text-slate-800 flex items-center gap-1.5">
            <span>⚙️</span> 生成通道模式
          </span>
          <span class="text-xs text-slate-500">（两条通道共用同一套底层 Prompt 模板与单物微观特写契约）</span>
        </div>
        <el-radio-group
      ...
      <div class="bg-slate-50 border border-slate-200 rounded-xl p-3.5 space-y-3">
        <div class="flex items-center justify-between">
          <span class="text-xs font-bold text-slate-800 flex items-center gap-1.5">
            <span>🗡️</span> 当前道具资产上下文 (纯净实物契约)
          </span>
          <el-tag size="small" type="warning" effect="plain">单物微观特写契约</el-tag>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
          <div class="bg-white p-2 rounded border border-slate-100">
            <span class="text-slate-400 block mb-0.5">道具名称与类型</span>
            <span class="font-medium text-slate-800">{{ currentContext.name || '未命名道具' }} ({{ currentContext.propType || 'KEY_PROP' }})</span>
          </div>
          <div class="bg-white p-2 rounded border border-slate-100">
            <span class="text-slate-400 block mb-0.5">道具状态</span>
          ...
          <div class="bg-white p-2 rounded border border-slate-100 md:col-span-2">
            <span class="text-slate-400 block mb-0.5">中文特征描述 (原著细节与材质真实源)</span>
            <span class="font-medium text-slate-800 line-clamp-2">{{ currentContext.description || '暂未填写中文描述' }}</span>
          </div>
        </div>

        <!-- 关联短剧自动带入状态提示 -->
        <div v-if="dramaInfo" class="flex items-center justify-between bg-amber-50/80 border border-amber-200/80 rounded-lg px-3 py-2 text-xs text-amber-900">
          <div class="flex items-center gap-1.5 font-medium">
            <span>🎬</span>
            <span>关联短剧：《{{ dramaInfo.title }}》</span>
            <span class="text-amber-300">|</span>
            <span class="text-amber-700 font-normal">已自动带入短剧全局风格与视觉基调指南</span>
          </div>
        ...
        <div class="space-y-2.5 pt-1 border-t border-slate-200/60">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div v-else class="flex flex-col justify-center bg-amber-50/70 border border-amber-200/80 rounded px-2.5 py-1.5 text-xs text-amber-900">
          ...
          <div>
            <label class="block text-[11px] font-semibold text-slate-700 mb-1">
              ...
              <span class="text-[10px] text-slate-400 font-normal ml-1">自动继承短剧设定，光影/色调/微距质感深度融入</span>
            </label>
            <el-input
              v-model="styleTone"
              type="textarea"
              :rows="2"
              placeholder="如：微距静物特写、柔和暖调侧逆光、金属做旧氧化质感、高锐度微观材质（自动带入短剧配置）"
              size="small"
              clearable
              :disabled="isStreaming"
              @change="onStyleParamsChange"
      ...
      <div v-if="executionMode === 'MANUAL'" class="space-y-3">
        <!-- 步骤提示导航条 -->
        <div class="flex items-center justify-between bg-amber-50/90 border border-amber-200 rounded-xl px-3.5 py-2 text-xs text-amber-950">
          <div class="flex items-center gap-2 sm:gap-4 flex-wrap">
            <span class="font-bold flex items-center gap-1">
              <span class="w-4 h-4 rounded-full bg-amber-600 text-white flex items-center justify-center text-[10px]">1</span>
              生成并复制完整 Prompt
            </span>
            <span class="text-amber-300">→</span>
            <span class="font-bold flex items-center gap-1">
              <span class="w-4 h-4 rounded-full bg-amber-600 text-white flex items-center justify-center text-[10px]">2</span>
              粘贴到外部 AI 获取 JSON
            </span>
            <span class="text-amber-300">→</span>
            <span class="font-bold flex items-center gap-1">
              <span class="w-4 h-4 rounded-full bg-amber-600 text-white flex items-center justify-center text-[10px]">3</span>
              粘贴结果并校验回填
            </span>
          </div>
          <el-button
          ...
          <el-tab-pane label="📋 任务 Prompt" name="prompt">
            <div class="space-y-2.5">
              <div class="flex items-center justify-between text-xs text-slate-500 flex-wrap gap-2">
                <div class="flex items-center gap-2">
                  <span>版本: <el-tag size="small" type="info">{{ promptPackage?.templateVersion || 'prop-prompt-v1' }}</el-tag></span>
            ...
            <div class="space-y-3">
              ...
              <div v-if="validationInfo" class="space-y-1.5">
                ...
                <el-alert
                  ...
                  show-icon
                  :closable="false"
                  title="注意：道具上下文指纹与生成任务时不一致，设定可能已发生变更，请核对预览内容。"
                />
              </div>
        ...
        <div class="flex items-center justify-between pb-2 mb-2 border-b border-slate-700/60 text-xs">
          <div class="flex items-center gap-2">
            ...
            <span class="font-mono font-medium">
              {{ isStreaming ? '⚡ AI 实时构思与流式解析道具提示词...' : '✓ AI 道具提示词构思生成完毕' }}
            </span>
          </div>
      ...
      <div v-if="previewResult" class="space-y-3 pt-1">
        <div class="flex items-center justify-between">
          <span class="text-xs font-bold text-slate-800 flex items-center gap-1.5">
            <span>✨</span> 道具专属提示词预览 (确认后可一键采纳回填到抽屉)
          </span>
        ...
        <div class="space-y-3 bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs">
          <div>
            <div class="flex items-center justify-between mb-1">
              <label class="text-xs font-semibold text-slate-700">🗡️ 核心道具微观 Prompt (propPrompt)</label>
              <span class="text-[11px] text-slate-400">材质肌理/工艺纹理/磨损光泽/纯净微距</span>
            </div>
            <el-input
              v-model="previewResult.propPrompt"
              type="textarea"
              :rows="4"
              class="font-mono text-xs"
            />
          ...
          <div>
            <div class="flex items-center justify-between mb-1">
              ...
              <span class="text-[11px] text-slate-400">绝不含持物人手/人物肢体/宏观大背景/现代塑料感</span>
            </div>
            <el-input
      ...
      <div class="flex items-center justify-between">
        <div class="text-xs text-slate-400">
          ...
          <span v-else-if="previewResult" class="text-slate-500">
            满意请点击右侧「确认采纳并回填」更新道具表单
          </span>
          <span v-else-if="executionMode === 'MANUAL'" class="text-amber-600">
...
import type {
  ...
  PropPromptParseRequestDTO,
  PropPromptValidationResult
} from '@/types/resource'
import type { AiProviderVO, AiModel } from '@/types/ai-provider'
import type { Drama } from '@/types/drama'

const emit = defineEmits<{
  (e: 'apply', result: PropPromptDeriveVO): void
...
const currentContext = reactive<Partial<ResProp>>({})
const previewResult = ref<PropPromptDeriveVO | null>(null)
...
const modalTitle = computed(() => {
  return executionMode.value === 'API'
    ? '⚡ AI 依据道具设定智能生成专属 Prompt (API 模式)'
    : '📋 外部 AI 手工生成道具专属 Prompt (零 API 费用)'
})

function open(prop: Partial<ResProp>) {
  Object.assign(currentContext, prop)
  stylePreset.value = ''
  styleTone.value = ''
  skillNames.value = []
  ...
  if (prop.dramaId && prop.dramaId !== 0 && prop.dramaId !== '0') {
    dramaApi.getById(prop.dramaId).then(res => {
      dramaInfo.value = res
      if (res) {
        if (res.stylePreset) {
...
function buildDerivePayload(): PropPromptDeriveDTO {
  return {
    propId: currentContext.id,
    dramaId: currentContext.dramaId,
    name: currentContext.name,
    propType: currentContext.propType,
...
async function loadPromptPackage() {
  ...
  try {
    ...
    const res = await propApi.derivePromptPackage(payload)
...
async function handleParseManualResponse() {
  ...
  try {
    const request: PropPromptParseRequestDTO = {
      propId: currentContext.id,
      dramaId: currentContext.dramaId,
      name: currentContext.name,
      propType: currentContext.propType,
    ...
    const res: PropPromptValidationResult = await propApi.parseDerivedPrompt(request)
    ...
    if (res.result) {
      ...
      ElMessage.warning('未能提取到有效的道具提示词结构')
    }
  } catch (err: any) {
...
function handleViewJsonExample() {
  const example = `{
  "propPrompt": "cinematic macro shot of an ancient dragon-engraved bronze sword, weathered jade pommel, intricate Damascus steel blade, cold metallic glint, dramatic rim lighting, dark minimalist background, 8k resolution, photorealistic",
  "negativePrompt": "holding hands, fingers, human, person, body, blurred, plastic texture, low quality, modern background"
}`
  ElMessageBox.alert(
...
async function handleStartStream() {
  if (!currentContext.name && !currentContext.description) {
    ElMessage.warning('请确保至少填写了道具名称或道具描述')
    return
  }
  ...
  try {
    await propApi.derivePromptsStream(payload, {
      ...
      onError(err) {
        ElMessage.error(err.message || 'AI 衍生道具提示词失败')
        cleanupStream()
      },
      onDone() {
        cleanupStream()
...
function handleConfirmApply() {
  ...
  visible.value = false
  emit('panel-close')
  ElMessage.success('已采纳 AI 生成的道具提示词')
}
